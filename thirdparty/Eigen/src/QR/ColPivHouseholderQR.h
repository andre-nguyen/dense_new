...
ColPivHouseholderQR<MatrixType>& ColPivHouseholderQR<MatrixType>::compute(
  ...
  for (Index k = 0; k < size; ++k) {
    ...
    // if the current biggest column is smaller than epsilon times the initial
    // biggest column,
    // terminate to avoid generating nan/inf values.
    // Note that here, if we test instead for "biggest == 0", we get a failure
    // every 1000 (or so)
    // repetitions of the unit test, with the result of solve() filled with
