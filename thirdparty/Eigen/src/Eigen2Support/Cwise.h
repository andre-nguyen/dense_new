...
#define EIGEN_CWISE_BINOP_RETURN_TYPE(OP)                              \
  CwiseBinaryOp<OP<typename internal::traits<ExpressionType>::Scalar>, \
                ExpressionType, OtherDerived>

/** \internal
  * convenient macro to defined the return type of a cwise unary operation */
#define EIGEN_CWISE_UNOP_RETURN_TYPE(OP)                              \
  CwiseUnaryOp<OP<typename internal::traits<ExpressionType>::Scalar>, \
               ExpressionType>

/** \internal
  * convenient macro to defined the return type of a cwise comparison to a
 * scalar */
#define EIGEN_CWISE_COMP_TO_SCALAR_RETURN_TYPE(OP)                     \
  CwiseBinaryOp<OP<typename internal::traits<ExpressionType>::Scalar>, \
                ExpressionType, typename ExpressionType::ConstantReturnType>

/** \class Cwise
  *
  * \brief Pseudo expression providing additional coefficient-wise operations
  *
  * \param ExpressionType the type of the object on which to do coefficient-wise
  *operations
  *
  * This class represents an expression with additional coefficient-wise
  *features.
...
class Cwise {
 public:
  ...
  typedef CwiseUnaryOp<internal::scalar_add_op<Scalar>, ExpressionType>
      ScalarAddReturnType;
  ...
  const EIGEN_CWISE_BINOP_RETURN_TYPE(internal::scalar_min_op)(min)(
      const MatrixBase<OtherDerived>& other) const {
    return EIGEN_CWISE_BINOP_RETURN_TYPE(internal::scalar_min_op)(
  ...
  const EIGEN_CWISE_BINOP_RETURN_TYPE(internal::scalar_max_op)(max)(
      const MatrixBase<OtherDerived>& other) const {
    return EIGEN_CWISE_BINOP_RETURN_TYPE(internal::scalar_max_op)(
        _expression(), other.derived());
  }
  ...
  const EIGEN_CWISE_UNOP_RETURN_TYPE(internal::scalar_inverse_op)
      inverse() const;
  ...
  const EIGEN_CWISE_UNOP_RETURN_TYPE(internal::scalar_pow_op)
      pow(const Scalar& exponent) const;

  const ScalarAddReturnType operator+(const Scalar& scalar) const;

  /** \relates Cwise */
  friend const ScalarAddReturnType operator+(const Scalar& scalar,
                                             const Cwise& mat) {
    return mat + scalar;
  }
  ...
  const EIGEN_CWISE_COMP_TO_SCALAR_RETURN_TYPE(std::less) operator<(
      Scalar s) const;

  const EIGEN_CWISE_COMP_TO_SCALAR_RETURN_TYPE(std::less_equal) operator<=(
      Scalar s) const;

  const EIGEN_CWISE_COMP_TO_SCALAR_RETURN_TYPE(std::greater) operator>(
      Scalar s) const;
...
/** \returns a Cwise wrapper of *this providing additional coefficient-wise
  *operations
  *
  * Example: \include MatrixBase_cwise_const.cpp
  * Output: \verbinclude MatrixBase_cwise_const.out
