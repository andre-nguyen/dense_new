...
// Note that we have to pass Dim and HDim because it is not allowed to use a
// template
// parameter to define a template specialization. To be more precise, in the
// following
...
/** \geometry_module \ingroup Geometry_Module
  ...
  *
  * The homography is internally represented and stored as a (Dim+1)^2 matrix
  *which
...
class Transform {
 ...
 public:
  ...
  template <typename OtherDerived,
            bool BigMatrix>  // MSVC 2005 will commit suicide if BigMatrix has a
                             // default value
  ...
  inline explicit Transform(const MatrixBase<OtherDerived>& other) {
    construct_from_matrix<OtherDerived, int(OtherDerived::RowsAtCompileTime) ==
                                            Dim>::run(this, other);
  }
  ...
  /** \returns a read-only expression of the linear (linear) part of the
   * transformation */
  inline ConstLinearPart linear() const {
    return m_matrix.template block<Dim, Dim>(0, 0);
  }
  /** \returns a writable expression of the linear (linear) part of the
   * transformation */
  ...
  */
  ...
  template <typename OtherDerived>
  inline const typename ei_transform_product_impl<OtherDerived, _Dim,
                                                  _Dim + 1>::ResultType
  operator*(const MatrixBase<OtherDerived>& other) const {
    return ei_transform_product_impl<OtherDerived, Dim, HDim>::run(
        *this, other.derived());
  }
  ...
  friend inline Transform operator*(const LinearMatrixType& mat,
                                    const Transform& t) {
    Transform res = t;
  ...
  inline
      typename internal::cast_return_type<Transform,
                                          Transform<NewScalarType, Dim> >::type
      cast() const {
    return typename internal::cast_return_type<
        Transform, Transform<NewScalarType, Dim> >::type(*this);
  }
...
/** Set \c *this from a QMatrix assuming the dimension is 2.
  ...
  */
template <typename Scalar, int Dim>
Transform<Scalar, Dim>& Transform<Scalar, Dim>::operator=(
    const QMatrix& other) {
...
template <typename Scalar, int Dim>
template <typename OtherDerived>
Transform<Scalar, Dim>& Transform<Scalar, Dim>::scale(
    const MatrixBase<OtherDerived>& other) {
...
template <typename Scalar, int Dim>
template <typename OtherDerived>
Transform<Scalar, Dim>& Transform<Scalar, Dim>::prescale(
    const MatrixBase<OtherDerived>& other) {
  EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(OtherDerived, int(Dim))
...
template <typename Scalar, int Dim>
template <typename OtherDerived>
Transform<Scalar, Dim>& Transform<Scalar, Dim>::translate(
    const MatrixBase<OtherDerived>& other) {
  EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(OtherDerived, int(Dim))
...
template <typename Scalar, int Dim>
template <typename RotationType>
Transform<Scalar, Dim>& Transform<Scalar, Dim>::rotate(
...
template <typename Scalar, int Dim>
template <typename RotationType>
Transform<Scalar, Dim>& Transform<Scalar, Dim>::prerotate(
    const RotationType& rotation) {
  m_matrix.template block<Dim, HDim>(0, 0) =
      ei_toRotationMatrix<Scalar, Dim>(rotation) *
      m_matrix.template block<Dim, HDim>(0, 0);
  return *this;
}
...
void Transform<Scalar, Dim>::computeRotationScaling(
  ...
  if (scaling) {
    scaling->noalias() =
        svd.matrixV() * sv.asDiagonal() * svd.matrixV().adjoint();
  }
  if (rotation) {
...
template <typename Scalar, int Dim>
template <typename ScalingMatrixType, typename RotationMatrixType>
void Transform<Scalar, Dim>::computeScalingRotation(
  ...
  if (scaling) {
    scaling->noalias() =
        svd.matrixU() * sv.asDiagonal() * svd.matrixU().adjoint();
  }
  if (rotation) {
    LinearMatrixType m(svd.matrixU());
    m.col(0) /= x;
...
struct ei_transform_product_impl<Other, Dim, HDim, Dim, 1> {
  ...
  static ResultType run(const TransformType& tr, const Other& other) {
    return ((tr.linear() * other) + tr.translation()) *
           (Scalar(1) /
            ((tr.matrix().template block<1, Dim>(Dim, 0) * other).coeff(0) +
