...
#if defined(ENABLE_SSE)
  inline void updateSSE(const __m128& J1, const __m128& J2, const __m128& J3,
                        const __m128& J4, const __m128& res,
                        const __m128& weight) {
    // A.noalias() += J * J.transpose() * weight;
    __m128 J1w = _mm_mul_ps(J1, weight);
    _mm_store_ps(SSEData + 4 * 0,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 0), _mm_mul_ps(J1w, J1)));
    _mm_store_ps(SSEData + 4 * 1,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 1), _mm_mul_ps(J1w, J2)));
    _mm_store_ps(SSEData + 4 * 2,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 2), _mm_mul_ps(J1w, J3)));
    _mm_store_ps(SSEData + 4 * 3,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 3), _mm_mul_ps(J1w, J4)));

    __m128 J2w = _mm_mul_ps(J2, weight);
    _mm_store_ps(SSEData + 4 * 4,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 4), _mm_mul_ps(J2w, J2)));
    _mm_store_ps(SSEData + 4 * 5,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 5), _mm_mul_ps(J2w, J3)));
    _mm_store_ps(SSEData + 4 * 6,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 6), _mm_mul_ps(J2w, J4)));

    __m128 J3w = _mm_mul_ps(J3, weight);
    _mm_store_ps(SSEData + 4 * 7,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 7), _mm_mul_ps(J3w, J3)));
    _mm_store_ps(SSEData + 4 * 8,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 8), _mm_mul_ps(J3w, J4)));

    __m128 J4w = _mm_mul_ps(J4, weight);
    _mm_store_ps(SSEData + 4 * 9,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 9), _mm_mul_ps(J4w, J4)));

    // b.noalias() -= J * (res * weight);
    __m128 resw = _mm_mul_ps(res, weight);
    _mm_store_ps(SSEData + 4 * 10, _mm_add_ps(_mm_load_ps(SSEData + 4 * 10),
                                              _mm_mul_ps(resw, J1)));
    _mm_store_ps(SSEData + 4 * 11, _mm_add_ps(_mm_load_ps(SSEData + 4 * 11),
                                              _mm_mul_ps(resw, J2)));
    _mm_store_ps(SSEData + 4 * 12, _mm_add_ps(_mm_load_ps(SSEData + 4 * 12),
                                              _mm_mul_ps(resw, J3)));
    _mm_store_ps(SSEData + 4 * 13, _mm_add_ps(_mm_load_ps(SSEData + 4 * 13),
                                              _mm_mul_ps(resw, J4)));

    // error += res * res * weight;
    _mm_store_ps(SSEData + 4 * 14, _mm_add_ps(_mm_load_ps(SSEData + 4 * 14),
                                              _mm_mul_ps(resw, res)));
...
/**
 * Builds 4dof LGS (used for depth-lgs, at it has only 7 non-zero entries in
 * jacobian)
 ...
 public:
  ...
  inline void updateSSE(const __m128& J1, const __m128& J2, const __m128& J3,
    ...
    _mm_store_ps(SSEData + 4 * 3,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 3), _mm_mul_ps(J1w, J4)));
    _mm_store_ps(SSEData + 4 * 4,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 4), _mm_mul_ps(J1w, J5)));
    _mm_store_ps(SSEData + 4 * 5,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 5), _mm_mul_ps(J1w, J6)));

    __m128 J2w = _mm_mul_ps(J2, weight);
    _mm_store_ps(SSEData + 4 * 6,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 6), _mm_mul_ps(J2w, J2)));
    _mm_store_ps(SSEData + 4 * 7,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 7), _mm_mul_ps(J2w, J3)));
    _mm_store_ps(SSEData + 4 * 8,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 8), _mm_mul_ps(J2w, J4)));
    _mm_store_ps(SSEData + 4 * 9,
                 _mm_add_ps(_mm_load_ps(SSEData + 4 * 9), _mm_mul_ps(J2w, J5)));
    _mm_store_ps(SSEData + 4 * 10, _mm_add_ps(_mm_load_ps(SSEData + 4 * 10),
                                              _mm_mul_ps(J2w, J6)));
    ...
    __m128 J4w = _mm_mul_ps(J4, weight);
    _mm_store_ps(SSEData + 4 * 15, _mm_add_ps(_mm_load_ps(SSEData + 4 * 15),
                                              _mm_mul_ps(J4w, J4)));
    _mm_store_ps(SSEData + 4 * 16, _mm_add_ps(_mm_load_ps(SSEData + 4 * 16),
                                              _mm_mul_ps(J4w, J5)));
    _mm_store_ps(SSEData + 4 * 17, _mm_add_ps(_mm_load_ps(SSEData + 4 * 17),
                                              _mm_mul_ps(J4w, J6)));

    __m128 J5w = _mm_mul_ps(J5, weight);
    _mm_store_ps(SSEData + 4 * 18, _mm_add_ps(_mm_load_ps(SSEData + 4 * 18),
                                              _mm_mul_ps(J5w, J5)));
    _mm_store_ps(SSEData + 4 * 19, _mm_add_ps(_mm_load_ps(SSEData + 4 * 19),
                                              _mm_mul_ps(J5w, J6)));

    __m128 J6w = _mm_mul_ps(J6, weight);
    _mm_store_ps(SSEData + 4 * 20, _mm_add_ps(_mm_load_ps(SSEData + 4 * 20),
                                              _mm_mul_ps(J6w, J6)));

    // b.noalias() -= J * (res * weight);
    __m128 resw = _mm_mul_ps(res, weight);
    _mm_store_ps(SSEData + 4 * 21, _mm_add_ps(_mm_load_ps(SSEData + 4 * 21),
                                              _mm_mul_ps(resw, J1)));
    _mm_store_ps(SSEData + 4 * 22, _mm_add_ps(_mm_load_ps(SSEData + 4 * 22),
                                              _mm_mul_ps(resw, J2)));
    _mm_store_ps(SSEData + 4 * 23, _mm_add_ps(_mm_load_ps(SSEData + 4 * 23),
                                              _mm_mul_ps(resw, J3)));
    _mm_store_ps(SSEData + 4 * 24, _mm_add_ps(_mm_load_ps(SSEData + 4 * 24),
                                              _mm_mul_ps(resw, J4)));
    _mm_store_ps(SSEData + 4 * 25, _mm_add_ps(_mm_load_ps(SSEData + 4 * 25),
                                              _mm_mul_ps(resw, J5)));
    _mm_store_ps(SSEData + 4 * 26, _mm_add_ps(_mm_load_ps(SSEData + 4 * 26),
                                              _mm_mul_ps(resw, J6)));

    // error += res * res * weight;
    _mm_store_ps(SSEData + 4 * 27, _mm_add_ps(_mm_load_ps(SSEData + 4 * 27),
                                              _mm_mul_ps(resw, res)));
...
class LGS7 {
 public:
  ...
  void initializeFrom(const LGS6& ls6, const LGS4& ls4) {
    ...
    for (int i = 0; i < 4; i++) {
      b[remap[i]] += ls4.b[i];
      for (int j = 0; j < 4; j++) A(remap[i], remap[j]) += ls4.A(i, j);
    }
