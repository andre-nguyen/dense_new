...
LiveSLAMWrapper::LiveSLAMWrapper(std::string packagePath, ros::NodeHandle& _nh,
...
LiveSLAMWrapper::~LiveSLAMWrapper() {
  if (monoOdometry != 0) delete monoOdometry;
  if (outFile.is_open()) {
    outFile.flush();
    outFile.close();
  }
...
void LiveSLAMWrapper::popAndSetGravity() {
  ...
  pubOdometry(Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
              Eigen::Matrix3d::Identity(), monoOdometry->pub_odometry,
              monoOdometry->pub_pose, 0, R_vi_2_odometry, true, tImage);
...
void LiveSLAMWrapper::pubCameraLink() {
  ...
  for (int i = 0; i < slidingWindowSize; i++) {
    ...
    cv::putText(linkListMap, boost::to_string(i), locations[i],
  ...
  for (int i = 0; i < monoOdometry->numOfState; i++) {
    ...
    list<int>::iterator iter =
        monoOdometry->slidingWindow[idx]->cameraLinkList.begin();
    for (; iter != monoOdometry->slidingWindow[idx]->cameraLinkList.end();
         iter++) {
      int linkID = *iter;
      cv::line(linkListMap, locations[idx], locations[linkID],
...
void LiveSLAMWrapper::pubPointCloud(int num, ros::Time imageTimeStamp,
  ...
  const float* pyrIdepthVarSource =
      monoOdometry->currentKeyFrame->idepthVar(level);
  Eigen::Vector3f posDataPT;
  Eigen::Vector3f posDataOutput;
  Eigen::Matrix3f R_output;
  R_output << R_vi_2_odometry(0, 0), R_vi_2_odometry(0, 1),
      R_vi_2_odometry(0, 2), R_vi_2_odometry(1, 0), R_vi_2_odometry(1, 1),
      R_vi_2_odometry(1, 2), R_vi_2_odometry(2, 0), R_vi_2_odometry(2, 1),
      R_vi_2_odometry(2, 2);
...
void LiveSLAMWrapper::BALoop() {
  ...
  while (nh.ok()) {
    ...
    int ttt =
        (monoOdometry->frameInfoListTail - monoOdometry->frameInfoListHead);
    if (ttt < 0) {
    ...
    for (int sz;;) {
      ...
      if (monoOdometry->frameInfoList[monoOdometry->frameInfoListHead]
              .keyFrameFlag) {
        break;
      }
    }
    ros::Time imageTimeStamp =
        monoOdometry->frameInfoList[monoOdometry->frameInfoListHead].t;
    monoOdometry->frameInfoList_mtx.unlock();
    ...
    if (monoOdometry->frameInfoList[monoOdometry->frameInfoListHead]
      ...
      // set key frame
      monoOdometry->currentKeyFrame =
          monoOdometry->slidingWindow[monoOdometry->tail];
      monoOdometry->currentKeyFrame->keyFrameFlag = true;
      monoOdometry->currentKeyFrame->cameraLinkList.clear();
      ...
      if ((imageTimeStamp - lastLoopClorsureTime).toSec() > 0.18) {
        ...
        monoOdometry->setReprojectionListRelateToLastestKeyFrame(
            monoOdometry->head, preKeyFrameID,
            monoOdometry->slidingWindow[monoOdometry->tail].get());
        ROS_WARN(
            "loop closure link cost time: %f",
    ...
    if (monoOdometry->frameInfoList[monoOdometry->frameInfoListHead].trust) {
      ...
      monoOdometry->insertCameraLink(
          keyFrame, currentFrame,
          monoOdometry->frameInfoList[monoOdometry->frameInfoListHead].R_k_2_c,
          -monoOdometry->frameInfoList[monoOdometry->frameInfoListHead].T_k_2_c,
          monoOdometry->frameInfoList[monoOdometry->frameInfoListHead]
              .lastestATA);
    }
    ...
    //        cout << "[-BA]current Position: " <<
    //        currentFrame->T_bk_2_b0.transpose() << endl;
    //        cout << "[-BA]current Velocity: " <<
    //        currentFrame->v_bk.transpose() << endl;
    ...
    // cout << "[BA-]current Position: " << currentFrame->T_bk_2_b0.transpose()
    // << endl;
    // cout << "[BA-]current Velocity: " << currentFrame->v_bk.transpose() <<
    // endl;
...
#ifdef PRINT_DEBUG_INFO
    int colorFlag = 0;
    colorFlag = monoOdometry->frameInfoList[monoOdometry->frameInfoListHead]
                    .keyFrameFlag;
    if (monoOdometry->frameInfoList[monoOdometry->frameInfoListHead].trust ==
        false) {
      colorFlag = 2;
    ...
    pubPath(monoOdometry->slidingWindow[monoOdometry->tail]->T_bk_2_b0,
            colorFlag, monoOdometry->path_line, monoOdometry->pub_path,
            R_vi_2_odometry);
#endif
    ...
    Vector3d t_translation =
        R_vi_2_odometry *
        monoOdometry->slidingWindow[monoOdometry->tail]->T_bk_2_b0;
    Quaterniond t_q(monoOdometry->slidingWindow[monoOdometry->tail]->R_bk_2_b0);
    transform.setOrigin(
        tf::Vector3(t_translation(0), t_translation(1), t_translation(2)));
    tf::Quaternion q;
    Quaterniond tt_q(
        R_vi_2_odometry *
        monoOdometry->slidingWindow[monoOdometry->tail]->R_bk_2_b0 *
        R_vi_2_odometry.transpose());
    q.setW(tt_q.w());
    q.setX(tt_q.x());
    q.setY(tt_q.y());
    q.setZ(tt_q.z());
    ...
    //        Matrix3d tt_rotate =
    //        monoOdometry->slidingWindow[monoOdometry->tail]->R_bk_2_b0.transpose()
    //        *
    //                monoOdometry->slidingWindow[preIndex]->R_bk_2_b0 ;
    //        Quaterniond tt_q(tt_rotate) ;

    //        to_pub_info.x =
    //        monoOdometry->slidingWindow[monoOdometry->tail]->v_bk(0) ;
    //        to_pub_info.y =
    //        monoOdometry->slidingWindow[monoOdometry->tail]->v_bk(1) ;
    //        to_pub_info.z =
    //        monoOdometry->slidingWindow[monoOdometry->tail]->v_bk(2) ;
...
void LiveSLAMWrapper::Loop() {
  ...
  while (nh.ok()) {
    ...
    //        printf("%d %d\n", imuQueue.size() < 10,
    //        reverse_iterImu->header.stamp <= imageTimeStamp ) ;
    ...
    // std::cout << "[dt-imu] " << reverse_iterImu->header.stamp << " " <<
    // imuQueue.size() << std::endl ;
    ...
    //        if(!isInitialized)
    //        {
    //            monoOdometry->insertFrame(imageSeqNumber, image1,
    //            imageTimeStamp,
    //                                      Eigen::Matrix3d::Identity(),
    //                                      Eigen::Vector3d::Zero(),
    //                                      Eigen::Vector3d::Zero() );
    ...
    //            monoOdometry->currentKeyFrame = monoOdometry->slidingWindow[0]
    //            ;
    //            monoOdometry->currentKeyFrame->setDepthFromGroundTruth(
    //            (float*)depth.data ) ;
