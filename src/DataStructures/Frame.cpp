...
Frame::Frame(int id, int width, int height, const Eigen::Matrix3f& K,
             double timestamp, const unsigned char* image, Eigen::Matrix3d R,
             Eigen::Vector3d T, Eigen::Vector3d vel) {
...
Frame::~Frame() {
  ...
  if (permaRef_colorAndVarData != 0) delete permaRef_colorAndVarData;
  if (permaRef_posData != 0) delete permaRef_posData;
...
void Frame::calculateMeanInformation() {
  ...
  if (numMappablePixels < 0) maxGradients(0);
  ...
  for (int y = 0; y < height0; y++) {
    for (int x = 0; x < width0; x++) {
      if (x > 0 && x < width0 - 1 && y > 0 &&
          ...
          pyrMaxGradient[x + y * width0] >= MIN_ABS_GRAD_CREATE && *depth > 0) {
        meanIdepth += *depth;
        numSum++;
...
void Frame::release(int dataFlags, bool pyramidsOnly, bool invalidateOnly) {
  for (int level = (pyramidsOnly ? 1 : 0); level < PYRAMID_LEVELS; ++level) {
    if ((dataFlags & IMAGE) && data.imageValid[level]) {
      ...
      if (!invalidateOnly) releaseImage(level);
    }
    if ((dataFlags & GRADIENTS) && data.gradientsValid[level]) {
      data.gradientsValid[level] = false;
      if (!invalidateOnly) releaseGradients(level);
    }
    if ((dataFlags & MAX_GRADIENTS) && data.maxGradientsValid[level]) {
      data.maxGradientsValid[level] = false;
      if (!invalidateOnly) releaseMaxGradients(level);
    }
    if ((dataFlags & IDEPTH) && data.idepthValid[level]) {
      data.idepthValid[level] = false;
      if (!invalidateOnly) releaseIDepth(level);
    }
    if ((dataFlags & IDEPTH_VAR) && data.idepthVarValid[level]) {
      data.idepthVarValid[level] = false;
      if (!invalidateOnly) releaseIDepthVar(level);
    }
  }
}
...
void Frame::buildImage(int level) {
  if (level == 0) {
    printf(
        "Frame::buildImage(0): Loading image from disk is not implemented yet! "
        "No-op.\n");
    return;
  }
...
void Frame::releaseImage(int level) {
  if (level == 0) {
    printf(
        "Frame::releaseImage(0): Storing image on disk is not supported yet! "
        "No-op.\n");
    return;
  }
  FrameMemory::getInstance().returnBuffer(data.image[level]);
...
void Frame::buildGradients(int level) {
  ...
  if (data.gradientsValid[level]) return;
...
void Frame::buildMaxGradients(int level) {
  ...
  for (; maxgrad_pt < maxgrad_pt_max; maxgrad_pt++, maxgrad_t_pt++) {
    ...
    if (g1 < g3) {
      *maxgrad_pt = g3;
      if (g3 >= MIN_ABS_GRAD_CREATE) numMappablePixels++;
    } else {
      *maxgrad_pt = g1;
      if (g1 >= MIN_ABS_GRAD_CREATE) numMappablePixels++;
    }
  }

  if (level == 0) this->numMappablePixels = numMappablePixels;
...
void Frame::buildIDepthAndIDepthVar(int level) {
  ...
  if (level == 0) {
    printf(
        "Frame::buildIDepthAndIDepthVar(0): Loading depth from disk is not "
  ...
  if (data.idepthValid[level] && data.idepthVarValid[level]) return;
...
void Frame::releaseIDepth(int level) {
  if (level == 0) {
    printf(
        "Frame::releaseIDepth(0): Storing depth on disk is not supported yet! "
        "No-op.\n");
    return;
  }
...
void Frame::releaseIDepthVar(int level) {
  if (level == 0) {
    printf(
        "Frame::releaseIDepthVar(0): Storing depth variance on disk is not "
        "supported yet! No-op.\n");
    return;
  }
  FrameMemory::getInstance().returnBuffer(data.idepthVar[level]);
