...
void FrameMemory::releaseBuffes() {
  ...
  if (printMemoryDebugInfo) printf("released %.1f MB!\n", total / (1000000.0f));
}
...
boost::shared_lock<boost::shared_mutex> FrameMemory::activateFrame(
    Frame* frame) {
...
void FrameMemory::deactivateFrame(Frame* frame) {
  ...
  while (!frame->minimizeInMemory())
    printf(
        "cannot deactivateFrame frame %d, as some acvite-lock is lingering. "
        "May cause deadlock!\n",
...
void FrameMemory::pruneActiveFrames() {
  ...
  while ((int)activeFrames.size() > maxLoopClosureCandidates + 20) {
    if (!activeFrames.back()->minimizeInMemory()) {
      if (!activeFrames.back()->minimizeInMemory()) {
        printf(
            "failed to minimize frame %d twice. maybe some active-lock is "
            "lingering?\n",
