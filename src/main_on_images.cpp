...
int getFile(std::string source, std::vector<std::string> &files) {
  ...
  if (f.good() && f.is_open()) {
    ...
    for (unsigned int i = 0; i < files.size(); i++) {
      if (files[i].at(0) != '/') files[i] = prefix + "/" + files[i];
    }
...
int main(int argc, char **argv) {
  ...
  dynamic_reconfigure::Server<lsd_slam_core::LSDParamsConfig> srv(
      ros::NodeHandle("~"));
  srv.setCallback(dynConfCb);

  dynamic_reconfigure::Server<lsd_slam_core::LSDDebugParamsConfig> srvDebug(
  ...
  // if no undistortion is required, the undistorter will just pass images
  // through.
  ...
  for (unsigned int i = 0; i < files.size(); i++) {
    ...
    if (imageDist.rows != h_inp || imageDist.cols != w_inp) {
      if (imageDist.rows * imageDist.cols == 0)
        ...
        printf(
            "image %s has wrong dimensions - expecting %d x %d, found %d x %d. "
            "Skipping.\n",
