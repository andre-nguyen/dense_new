...
class Math {
 public:
  ...
  MatrixXd JacobianF(const Matrix3d& R, const Vector3d& a, const Vector3d& w,
                     double dt) {
  ...
  MatrixXd JacobianG(const Matrix3d& R, const Vector3d& a, const Vector3d& w,
                     double dt) {
  ...
  VectorXd ResidualImu(const Vector3d& p1, const Vector3d& v1,
                       const Matrix3d& R1, const Vector3d& p2,
                       const Vector3d& v2, const Matrix3d& R2,
                       const Vector3d& g, double dt, const Vector3d& dp,
                       const Vector3d& dv, const Matrix3d& dR) {
    VectorXd r = VectorXd::Zero(9);
    r.segment<3>(0) =
        (R1.transpose() * (p2 - p1 + g * dt * dt / 2.0) - v1 * dt) - dp;
    r.segment<3>(3) = (R1.transpose() * (R2 * v2 + g * dt) - v1) - dv;
    r.segment<3>(6) =
        Quaterniond(dR.transpose() * R1.transpose() * R2).vec() * 2.0;
    return r;
  }

  MatrixXd JacobianImu(const Vector3d& p1, const Vector3d& v1,
                       const Matrix3d& R1, const Vector3d& p2,
                       const Vector3d& v2, const Matrix3d& R2,
  ...
  VectorXd ResidualCamera(const Vector3d& p1, const Matrix3d& R1,
                          const Vector3d& p2, const Matrix3d& R2, double d1,
                          const Vector2d& uv1, const Vector2d& uv2) {
    Matrix3d Ric;
    Ric << 0, -1, 0, 0, 0, -1, 1, 0, 0;
    Vector3d uv1h = Vector3d(uv1(0), uv1(1), 1);
    Vector3d f =
        Ric * R2.transpose() * (p1 - p2 + d1 * R1 * Ric.transpose() * uv1h);
  ...
  MatrixXd JacobianCamera(const Vector3d& p1, const Matrix3d& R1,
    ...
    JF.block<3, 3>(0, 6) =
        -R2.transpose() * R1 * Skew(d1 * Ric.transpose() * uv1h);
    JF.block<3, 3>(0, 9) = -R2.transpose();
    JF.block<3, 3>(0, 15) =
        Skew(R2.transpose() * (p1 - p2 + d1 * R1 * Ric.transpose() * uv1h));
  ...
  VectorXd ResidualDenseTracking(const Vector3d& pi, const Matrix3d& Ri,
    ...
    r.segment<3>(6) =
        Quaterniond(Rij.transpose() * Rj.transpose() * Ri).vec() * 2.0;
