...
#define UNZERO(val) \
  (val < 0 ? (val > -1e-10 ? -1e-10 : val) : (val < 1e-10 ? 1e-10 : val))
...
// const Eigen::Matrix3d acc_cov = 0.23/1000.0*9.8 * 0.23/1000.0*9.8 * 200 *
// Eigen::Matrix3d::Identity();
// const Eigen::Matrix3d gra_cov = 0.23/1000.0*9.8 * 0.23/1000.0*9.8 * 200 *
// Eigen::Matrix3d::Identity();
// const Eigen::Matrix3d gyr_cov = 0.0135/180.0*PI * 0.0135/180.0*PI * 25 *
// Eigen::Matrix3d::Identity();
...
// validity can take values between 0 and X, where X depends on the abs.
...
#define VALIDITY_COUNTER_MAX_VARIABLE \
  (250.0f)  // validity will never be higher than this

#define VALIDITY_COUNTER_INC \
  5  // validity is increased by this on sucessfull stereo
#define VALIDITY_COUNTER_DEC \
  5  // validity is decreased by this on failed stereo
#define VALIDITY_COUNTER_INITIAL_OBSERVE \
  5  // initial validity for first observations

#define VAL_SUM_MIN_FOR_CREATE \
  (30)  // minimal summed validity over 5x5 region to create a new hypothesis
        // for non-blacklisted pixel (hole-filling)
#define VAL_SUM_MIN_FOR_KEEP \
  (24)  // minimal summed validity over 5x5 region to keep hypothesis
        // (regularization)
#define VAL_SUM_MIN_FOR_UNBLACKLIST \
  (100)  // if summed validity surpasses this, a pixel is un-blacklisted.

#define MIN_BLACKLIST \
  -1  // if blacklist is SMALLER than this, pixel gets ignored. blacklist starts
      // with 0.

/** ============== Depth Variance Handeling ======================= */
#define SUCC_VAR_INC_FAC \
  (1.01f)  // before an ekf-update, the variance is increased by this factor.
#define FAIL_VAR_INC_FAC \
  1.1f  // after a failed stereo observation, the variance is increased by this
        // factor.
#define MAX_VAR \
  (0.5f * 0.5f)  // initial variance on creation - if variance becomes larter
                 // than this, hypothesis is removed.

#define VAR_GT_INIT_INITIAL \
  0.01f * 0.01f  // initial variance vor Ground Truth Initialization
#define VAR_RANDOM_INIT_INITIAL \
  (0.5f * MAX_VAR)  // initial variance vor Random Initialization
...
#define SE3TRACKING_MAX_LEVEL 3
...
#define MAX_ERROR_STEREO \
  (1300.0f)  // maximal photometric error for stereo to be successful (sum over
             // 5 squared intensity differences)
#define MIN_DISTANCE_ERROR_STEREO \
  (1.5f)  // minimal multiplicative difference to second-best match to not be
...
// basically determines the amount of spacial smoothing (small -> more
// smoothing).
#define REG_DIST_VAR \
...
/// Relative path of calibration file, map saving directory etc. for
/// live_odometry
...
class RunningStats {
 public:
  ...
  inline RunningStats() { setZero(); }

  inline void setZero() { memset(this, 0, sizeof(RunningStats)); }

  inline void add(RunningStats* r) {
    int* pt = (int*)this;
    int* pt_r = (int*)r;
    for (int i = 0; i < static_cast<int>(sizeof(RunningStats) / sizeof(int));
         i++)
      pt[i] += pt_r[i];
  }
};
...
class DenseDepthTrackerSettings {
 public:
  inline DenseDepthTrackerSettings() {
    ...
    if (PYRAMID_LEVELS > 6)
      printf(
          "WARNING: Sim3Tracker(): default settings are intended for a maximum "
          "of 6 levels!");
