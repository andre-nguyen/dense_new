...
struct MARGINALIZATION {
  ...
  MARGINALIZATION() {
    Ap = MatrixXd::Zero(variablesNumInState * slidingWindowSize,
                        variablesNumInState * slidingWindowSize);
  ...
  void marginalizeOutTheEnd() {
    // Schur complement
    MatrixXd BD_inv =
        Ap.block(0, STATE_SZ(size - 1), STATE_SZ(size - 1),
                 variablesNumInState) *
        Ap.block(STATE_SZ(size - 1), STATE_SZ(size - 1), variablesNumInState,
                 variablesNumInState).inverse();
    Ap.block(0, 0, STATE_SZ(size - 1), STATE_SZ(size - 1)) -=
        BD_inv *
        Ap.block(STATE_SZ(size - 1), 0, variablesNumInState,
                 STATE_SZ(size - 1));
  ...
  void popFrontState() {
    // swap cols
    Ap.block(0, STATE_SZ(size - 2), STATE_SZ(size), variablesNumInState)
        .swap(Ap.block(0, STATE_SZ(size - 1), STATE_SZ(size),
                       variablesNumInState));

    // swap rows
    bp.segment(STATE_SZ(size - 2), variablesNumInState)
        .swap(bp.segment(STATE_SZ(size - 1), variablesNumInState));
    Ap.block(STATE_SZ(size - 2), 0, variablesNumInState, STATE_SZ(size))
        .swap(Ap.block(STATE_SZ(size - 1), 0, variablesNumInState,
                       STATE_SZ(size)));
  ...
  void popEndState() {
    ...
    for (int i = 1; i < size; i++) {
      ...
      bp.segment(STATE_SZ(i - 1), variablesNumInState) =
          bp.segment(STATE_SZ(i), variablesNumInState);
    }
