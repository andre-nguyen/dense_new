...
inline void setPixelInCvMat(cv::Mat* mat, cv::Vec3b color, int xx, int yy,
  ...
  for (int x = xx * lvlFac; x < (xx + 1) * lvlFac && x < mat->size().width; x++)
    for (int y = yy * lvlFac; y < (yy + 1) * lvlFac && y < mat->size().height;
         y++)
      mat->at<cv::Vec3b>(y, x) = color;
}
...
inline float getRefFrameScore(float distanceSquared, float usage,
                              float KFDistWeight, float KFUsageWeight) {
  return distanceSquared * KFDistWeight * KFDistWeight +
         (1 - usage) * (1 - usage) * KFUsageWeight * KFUsageWeight;
}

inline void calculateDepthImage(cv::Mat disparity, cv::Mat& depthImage,
                                float baseline, float f) {
