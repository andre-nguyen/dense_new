...
void SlamSystem::debugDisplayDepthMap() {
  ...
  //    snprintf(buf1,200,"dens %2.0f%%; good %2.0f%%; scale %2.2f; res %2.1f/;
...
void SlamSystem::initRosPub() {
  ...
  pub_resudualMap =
      nh.advertise<sensor_msgs::Image>("denseVO/residualMap", 100);
  pub_reprojectMap =
      nh.advertise<sensor_msgs::Image>("denseVO/reprojectMap", 100);
  pub_gradientMapForDebug =
      nh.advertise<sensor_msgs::Image>("denseVO/debugMap", 100);
  pub_denseTracking = nh.advertise<geometry_msgs::Vector3>("denseVO/dt", 100);
  pub_angular_velocity =
      nh.advertise<geometry_msgs::Vector3>("denseVO/angular_velocity", 100);
  ;
  pub_linear_velocity =
      nh.advertise<geometry_msgs::Vector3>("denseVO/linear_velocity", 100);
  ;
...
void SlamSystem::generateDubugMap(Frame* currentFrame,
...
void SlamSystem::setDepthInit(cv::Mat img0, cv::Mat img1, double timeStamp,
                              int id) {
  cv::Mat disparity, depth;
  bm_(img1, img0, disparity, CV_32F);
  calculateDepthImage(disparity, depth, 0.11, K(0, 0));

  currentKeyFrame.reset(new Frame(
      id, width, height, K, timeStamp, img1.data, Eigen::Matrix3d::Identity(),
...
void SlamSystem::copyStateData(int preStateID) {
  ...
  //  slidingWindow[preStateID]->timeIntegral =
  //  slidingWindow[tail]->timeIntegral;

  //  slidingWindow[preStateID]->keyFrameFlag =
  //  slidingWindow[tail]->keyFrameFlag;
  ...
  //  {
  //    memcpy(slidingWindow[preStateID]->intensity[i],
  //    slidingWindow[tail]->intensity[i], n*m*sizeof(unsigned char));
  //    if (slidingWindow[tail]->keyFrameFlag )
  //    {
  //      memcpy(slidingWindow[preStateID]->depthImage[i],
  //      slidingWindow[tail]->depthImage[i], n*m*sizeof(float));
  //      memcpy(slidingWindow[preStateID]->gradientX[i],
  //      slidingWindow[tail]->gradientX[i], n*m*sizeof(double));
  //      memcpy(slidingWindow[preStateID]->gradientY[i],
  //      slidingWindow[tail]->gradientY[i], n*m*sizeof(double));

  //      slidingWindow[preStateID]->totalNumOfValidPixels[i] =
  //      slidingWindow[tail]->totalNumOfValidPixels[i];
  ...
  //      for (int j = 0; j < sz; j++)
  //      {
  //        slidingWindow[preStateID]->pixelInfo[i].Aij[j] =
  //        slidingWindow[tail]->pixelInfo[i].Aij[j];
  //        slidingWindow[preStateID]->pixelInfo[i].AijTAij[j] =
  //        slidingWindow[tail]->pixelInfo[i].AijTAij[j];
  //      }
  ////        slidingWindow[preStateID]->pixelInfo[i].Aij.swap(
  ///slidingWindow[tail]->pixelInfo[i].Aij );
  ////        slidingWindow[preStateID]->pixelInfo[i].AijTAij.swap(
  ///slidingWindow[tail]->pixelInfo[i].AijTAij );
  //      slidingWindow[preStateID]->pixelInfo[i].piList =
  //      slidingWindow[tail]->pixelInfo[i].piList ;
  //      slidingWindow[preStateID]->pixelInfo[i].intensity =
  //      slidingWindow[tail]->pixelInfo[i].intensity ;
  //      slidingWindow[preStateID]->pixelInfo[i].goodPixel =
  //      slidingWindow[tail]->pixelInfo[i].goodPixel ;
  //    }
  //    n >>= 1;
  //    m >>= 1;
  //  }
  ...
  for (int i = numOfState - 2; i >= 0; i--) {
    ...
    for (iter = slidingWindow[k]->cameraLinkList.begin();
         iter != slidingWindow[k]->cameraLinkList.end();) {
      if (*iter == preStateID) {
    ...
    for (iter = slidingWindow[k]->cameraLinkList.begin();
         iter != slidingWindow[k]->cameraLinkList.end(); iter++) {
      if (*iter == tail) {
        slidingWindow[k]->cameraLink[preStateID].R_bi_2_bj =
            slidingWindow[k]->cameraLink[tail].R_bi_2_bj;
        slidingWindow[k]->cameraLink[preStateID].T_bi_2_bj =
            slidingWindow[k]->cameraLink[tail].T_bi_2_bj;
        slidingWindow[k]->cameraLink[preStateID].P_inv =
            slidingWindow[k]->cameraLink[tail].P_inv;
        *iter = preStateID;
      }
    }
  }
  ...
  if (twoWayMarginalizatonFlag == false) {
    ...
    if (numOfState == slidingWindowSize) {
      ...
      if (slidingWindow[head]->imuLinkFlag) {
        ...
        VectorXd residualIMU = math.ResidualImu(
            T[k], vel[k], R[k], T[k1], vel[k1], R[k1], gravity_b0,
            slidingWindow[k]->timeIntegral, slidingWindow[k]->alpha_c_k,
            slidingWindow[k]->beta_c_k, slidingWindow[k]->R_k1_k);
        MatrixXd H_k1_2_k =
            math.JacobianImu(T[k], vel[k], R[k], T[k1], vel[k1], R[k1],
      ...
      VectorXd tmpHTb;
      list<int>::iterator iter =
          slidingWindow[currentStateID]->cameraLinkList.begin();
      for (; iter != slidingWindow[currentStateID]->cameraLinkList.end();
           iter++) {
        int linkID = *iter;

        H_i_2_j = math.JacobianDenseTracking(
            T[currentStateID], R[currentStateID], T[linkID], R[linkID]);
        residualCamera = math.ResidualDenseTracking(
            T[currentStateID], R[currentStateID], T[linkID], R[linkID],
            slidingWindow[currentStateID]->cameraLink[linkID].T_bi_2_bj,
            slidingWindow[currentStateID]->cameraLink[linkID].R_bi_2_bj);
        tmpP_inv.setZero();
        tmpP_inv.block(0, 0, 3, 3) =
            slidingWindow[currentStateID]->cameraLink[linkID].P_inv.block(0, 0,
                                                                          3, 3);
        tmpP_inv.block(0, 6, 3, 3) =
            slidingWindow[currentStateID]->cameraLink[linkID].P_inv.block(0, 3,
                                                                          3, 3);
        tmpP_inv.block(6, 0, 3, 3) =
            slidingWindow[currentStateID]->cameraLink[linkID].P_inv.block(3, 0,
                                                                          3, 3);
        tmpP_inv.block(6, 6, 3, 3) =
            slidingWindow[currentStateID]->cameraLink[linkID].P_inv.block(3, 3,
                                                                          3, 3);
        ...
        margin.Ap.block(currentStateIDIndex * 9, currentStateIDIndex * 9, 9,
                        9) += tmpHTH.block(0, 0, 9, 9);
        margin.Ap.block(currentStateIDIndex * 9, linkIDIndex * 9, 9, 9) +=
            tmpHTH.block(0, 9, 9, 9);
        margin.Ap.block(linkIDIndex * 9, currentStateIDIndex * 9, 9, 9) +=
            tmpHTH.block(9, 0, 9, 9);
        margin.Ap.block(linkIDIndex * 9, linkIDIndex * 9, 9, 9) +=
            tmpHTH.block(9, 9, 9, 9);
    ...
    for (int i = numOfState - 2; i >= numOfState - 3; i--) {
      ...
      residualIMU = math.ResidualImu(
          T[k], vel[k], R[k], T[k1], vel[k1], R[k1], gravity_b0,
          slidingWindow[k]->timeIntegral, slidingWindow[k]->alpha_c_k,
          slidingWindow[k]->beta_c_k, slidingWindow[k]->R_k1_k);
      H_k1_2_k = math.JacobianImu(T[k], vel[k], R[k], T[k1], vel[k1], R[k1],
                                  gravity_b0, slidingWindow[k]->timeIntegral);
      H_k1_2_k_T = H_k1_2_k.transpose();
      H_k1_2_k_T *= slidingWindow[k]->P_k.inverse();
...
void SlamSystem::BA() {
  ...
  for (int iterNum = 0; iterNum < maxIterationBA; iterNum++) {
    ...
    for (int i = numOfState - 2; i >= 0; i--) {
      ...
      dx.segment(STATE_SZ(i) + 6, 3) =
          Quaterniond(slidingWindow[k]->R_bk_2_b0.transpose() * R[k]).vec() *
          2.0;
    }
    HTH.block(0, 0, STATE_SZ(m_sz), STATE_SZ(m_sz)) +=
        margin.Ap.block(0, 0, STATE_SZ(m_sz), STATE_SZ(m_sz));
    HTb.segment(0, STATE_SZ(m_sz)) -=
        margin.Ap.block(0, 0, STATE_SZ(m_sz), STATE_SZ(m_sz)) * dx;
    ...
    for (int i = 0; i < numOfState; i++) {
      ...
      for (; iter != slidingWindow[currentStateID]->cameraLinkList.end();
        ...
        residualCamera = math.ResidualDenseTracking(
            ...
            slidingWindow[currentStateID]->cameraLink[linkID].T_bi_2_bj,
            slidingWindow[currentStateID]->cameraLink[linkID].R_bi_2_bj);
        //          residualCamera.segment(0, 3) =
        //          R[linkID].transpose()*(T[currentStateID] - T[linkID]) -
        //          slidingWindow[currentStateID]->cameraLink[linkID].T_bi_2_bj;
        //          residualCamera.segment(3, 3).setZero();
        //          residualCamera.segment(6, 3) = 2.0 *
        //          (Quaterniond(slidingWindow[currentStateID]->cameraLink[linkID].R_bi_2_bj.transpose())
        //          *
        //          Quaterniond(R[linkID].transpose()*R[currentStateID])).vec();
        ...
        HTH.block(currentStateIDIndex * 9, currentStateIDIndex * 9, 9, 9) +=
            tmpHTH.block(0, 0, 9, 9);
        HTH.block(currentStateIDIndex * 9, linkIDIndex * 9, 9, 9) +=
            tmpHTH.block(0, 9, 9, 9);
        HTH.block(linkIDIndex * 9, currentStateIDIndex * 9, 9, 9) +=
            tmpHTH.block(9, 0, 9, 9);
        HTH.block(linkIDIndex * 9, linkIDIndex * 9, 9, 9) +=
            tmpHTH.block(9, 9, 9, 9);
...
// cout << "iteration " << iterNum << "\n" << dx << endl;
  ...
  for (int i = numOfState - 2; i >= 0; i--) {
    ...
    r0.segment(STATE_SZ(i) + 6, 3) =
        Quaterniond(slidingWindow[k]->R_bk_2_b0.transpose() * R[k]).vec() * 2.0;
  }
  margin.bp.segment(0, STATE_SZ(m_sz)) +=
      margin.Ap.block(0, 0, STATE_SZ(m_sz), STATE_SZ(m_sz)) * r0;
...
void SlamSystem::insertFrame(int imageSeqNumber, cv::Mat img,
                             ros::Time imageTimeStamp, Matrix3d R, Vector3d T,
                             Vector3d vel) {
  tail++;
  numOfState++;
  //    if ( numOfState == slidingWindowSize ){
  ...
  slidingWindow[tail].reset(new Frame(
      tail, width, height, K, imageTimeStamp.toSec(), img.data, R, T, vel));
}

void SlamSystem::insertCameraLink(Frame* keyFrame, Frame* currentFrame,
                                  const Matrix3d& R_k_2_c,
                                  const Vector3d& T_k_2_c,
...
void SlamSystem::setReprojectionListRelateToLastestKeyFrame(int begin, int end,
                                                            Frame* current) {
  int num = end - begin;
  if (num < 0) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    if (slidingWindow[ref_id]->keyFrameFlag == false || trackFrameCnt > 10) {
      continue;
    }
    ...
    Eigen::Vector3d dd =
        (slidingWindow[ref_id]->T_bk_2_b0 - current->T_bk_2_b0) * distFac;
    if (dd.dot(dd) > distanceTH) continue;

    //        Eigen::Quaterniond qq(
    //        slidingWindow[ref_id]->R_bk_2_b0.transpose() * current->R_bk_2_b0)
    //        ;
    ...
    T_i_2_j = -slidingWindow[ref_id]->R_bk_2_b0.transpose() *
              (slidingWindow[ref_id]->T_bk_2_b0 - current->T_bk_2_b0);
    c2f_init.setRotationMatrix(R_i_2_j);
    c2f_init.translation() = T_i_2_j;
    trackerConstraint->trackFrameOnPermaref(
        current, slidingWindow[ref_id].get(), c2f_init);
    ...
    T_i_2_j = -current->R_bk_2_b0.transpose() *
              (current->T_bk_2_b0 - slidingWindow[ref_id]->T_bk_2_b0);
    c2f_init.setRotationMatrix(R_i_2_j);
    c2f_init.translation() = T_i_2_j;
    trackerConstraint->trackFrameOnPermaref(slidingWindow[ref_id].get(),
                                            current, c2f_init);
    if (trackerConstraint->trackingWasGood == false) {
    ...
    SE3 RefToFrame = trackerConstraint->trackFrame(trackingReferenceConstraint,
                                                   current, c2f_init);
    trackFrameCnt++;
    // float tracking_lastResidual = trackerConstraint->lastResidual;
    // float tracking_lastUsage = trackerConstraint->pointUsage;
    // float tracking_lastGoodPerBad = trackerConstraint->lastGoodCount /
    // (trackerConstraint->lastGoodCount + trackerConstraint->lastBadCount);
    float tracking_lastGoodPerTotal = trackerConstraint->lastGoodCount /
                                      (current->width(SE3TRACKING_MIN_LEVEL) *
                                       current->height(SE3TRACKING_MIN_LEVEL));
    Sophus::Vector3d dist =
        RefToFrame.translation() * slidingWindow[ref_id]->meanIdepth;
    float minVal = 1.0f;
    float lastTrackingClosenessScore =
        getRefFrameScore(dist.dot(dist), trackerConstraint->pointUsage,
    ...
    // ROS_WARN("[add link, from %d to %d]", slidingWindow[ref_id]->id(),
    // current->id() ) ;
...
void SlamSystem::processIMU(double dt, const Vector3d& linear_acceleration,
  ...
  F.block<3, 3>(3, 6) =
      -current->R_k1_k * vectorToSkewMatrix(linear_acceleration);
  ...
  current->P_k = (Matrix<double, 9, 9>::Identity() + dt * F) * current->P_k *
                     (Matrix<double, 9, 9>::Identity() + dt * F).transpose() +
                 (dt * G) * Q * (dt * G).transpose();
  // current->R_k1_k = current->R_k1_k*deltaR;
  current->alpha_c_k += current->beta_c_k * dt +
                        current->R_k1_k * linear_acceleration * dt * dt * 0.5;
...
void SlamSystem::trackFrame(cv::Mat img0, unsigned int frameID,
                            ...
                            const Eigen::Matrix3d R_i_2_c,
                            const Eigen::Vector3d T_i_2_c) {
  // Create new frame
  ...
  //    Matrix3d R_bk1_2_b0 = trackingReference->keyframe->R_bk_2_b0 *
  //    R_k_2_c.transpose();
  //    Vector3d T_bk1_2_b0 = trackingReference->keyframe->T_bk_2_b0 +
  //    R_bk1_2_b0*T_k_2_c ;
  ...
  msTrackFrame = (tv_end.tv_sec - tv_start.tv_sec) * 1000.0f +
                 (tv_end.tv_usec - tv_start.tv_usec) / 1000.0f;
  ...
  if (trackingReference->keyframe->numFramesTrackedOnThis > MIN_NUM_MAPPED) {
    Sophus::Vector3d dist =
        RefToFrame.translation() * currentKeyFrame->meanIdepth;
    float minVal = 1.0f;

    lastTrackingClosenessScore = getRefFrameScore(
        dist.dot(dist), tracker->pointUsage, KFDistWeight, KFUsageWeight);
    if (lastTrackingClosenessScore > minVal ||
        tracker->trackingWasGood == false ||
        tracking_lastGoodPerTotal < MIN_GOODPERALL_PIXEL) {
      ...
      //	if(enablePrintDebugInfo && printKeyframeSelectionInfo)
      //       printf("SKIPPD %d on %d! dist %.3f + usage %.3f = %.3f <
  ...
  tmpFrameInfo.T_k_2_c = R_i_2_c.transpose() * (r_k_2_c * T_i_2_c + t_k_2_c) -
                         R_i_2_c.transpose() * T_i_2_c;
#endif
